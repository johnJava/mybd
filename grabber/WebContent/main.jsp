...
<link rel="shortcut icon"
	href="http://images.5173cdn.com/images/img/ico.ico" />
...
<body>
	<div class="header">
					...
					<table width="947" border="0">
						<tbody>
							<tr>
								...
								<td>收货角色：</td>
								<td>选择客服：</td>
								<td><input type="button" id="startscan" value="开始扫描"
									style="width: 240px;" /></td>
							</tr>
							<tr>
								<td><input type="text" id="gameusername"
									style="width: 160px;" /></td>
								<td><select name="gametype" style="width: 160px;">
										<option value="dnf">dnf</option>
								</select></td>
								<td><input type="text" id="txtGameRole"
									style="width: 160px;" /></td>
								<td><select id="customer" style="width: 160px;">
										<option value="Star1">一星</option>
										<option value="Star5">五星</option>
								</select></td>
								<td><input type="button" id="stopscan" value="停止扫描"
									style="width: 240px;" /></td>
							</tr>
							<tr>
								<td>5173密码：</td>
								...
								<td>角色等级：</td>
								<td>价格大于等于：</td>
								<td rowspan="7"><textarea id="outputPanel"
										style="width: 240px; height: 175px; font-size: 10px; color:red"></textarea></td>
							</tr>
							<tr>
								<td><input type="password" id="password"
									style="width: 160px;" /></td>
								<td><select name="gamedistict" style="width: 160px;">
										<option value="alldis">全部区</option>
								</select></td>
								<td><input type="text" id="rolelevel" style="width: 160px;" /></td>
								<td><input type="text" id="minprice" style="width: 160px;" /></td>
							</tr>
							<tr>
								<td><input type="button" id="logingame" value="登录5173"
									style="width: 160px;" /></td>
								<td>选择服：</td>
								<td>联系电话：</td>
								<td>信誉小于等于：</td>
							...
							<tr>
								...
								<td><select name="gameservice" style="width: 160px;">
										<option value="alldis">全部服</option>
								</select></td>
								<td><input type="text" id="txtBuyerTel"
									style="width: 160px;" /></td>
								<td><select id="creditlevel" style="width: 160px;">
							...
							<tr>
								...
								<td><select name="selectprob" id="selectprob"
									style="width: 160px;">
										<option selected="selected" value="您父亲的名字是？">您父亲的名字是？</option>
										<option value="您母亲的名字是？">您母亲的名字是？</option>
										<option value="您的职业是？">您的职业是？</option>
								</select></td>
								<td><input type="text" id="answer" style="width: 160px;" /></td>
								<td><input type="text" id="txtBuyerQQ"
									style="width: 160px;" /></td>
							</tr>
							<tr>
		...
		<script type="text/javascript">
			function Login(loginType) {
				if (loginType == "common")
					window.location = "https://passport.5173.com/?UnDecode=1&returnUrl="
							+ escape(window.location.href);
				else if (loginType == "qq")
					window.location = "https://passport.5173.com/Partner/LoginFrom?UnDecode=1&appNo=qq&returnUrl="
							+ escape(window.location.href);
			}
		</script>
		<!--<p id="query"></p>
		...
		<div class="search-body clearfix">
			<div class="layout">
				...
				<div class="mainbody">
					...
					<script type="text/javascript"
						src="http://img01.5173cdn.com/fed/build/1.00/js/fed.share.js"></script>
					<div class="pdlist equipment_list">
						<ul id="headerForEquipment"
							class="tb_head equipment_head sort_bottom1">
							<li class="info">标题</li>
						...
						<div class="pdlistbox" id="resultlist"></div>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript" src="js/search/search_v36a.js"
		charset="utf-8"></script>
	<script type="text/javascript" src="js/fed-std/layer/v1/popup_layer.js"
		charset="utf-8"></script>
	<!-- WiseMedia检测代码 -->
	<script type="text/javascript"
		src="http://aw.kejet.net/t?p=K0&amp;c=vp&amp;er=1&amp;kd=1&amp;sid=1730880509812150273&amp;zid=1730880509812150272"></script>
	...
	<script type="text/javascript">
		...
		var console = {
			log : function(text) {
				if (text.indexOf("grabber_query") != -1) {
					query.innerHTML = text.substring(text
							.indexOf("grabber_query")
							+ "grabber_query".length);
				} else if (text.indexOf("grabber_avrtime") != -1) {
					avrtime.innerHTML = text.substring(text
							.indexOf("grabber_avrtime")
							+ "grabber_avrtime".length);
				} else if (text.indexOf("grabber_node") != -1) {
					infoPanel.innerHTML = text.substring(text
							.indexOf("grabber_node")
							+ "grabber_node".length)
							+ "<br>" + infoPanel.innerHTML;
				} else {
		...
		sendButton.onclick = function() {
			...
			if (!message)
				return;
			if (!demo.send(JSON.stringify(message)))
				return;
		};
		...
		function logingame() {
			...
			params.selectprob = document.getElementById('selectprob').value;
			params.answer = document.getElementById('answer').value;
